...
class Dataset:
    ...
    def load_data(self, name, fpath, na_values=None, entity_col=None, src_col=None):
        ...
        try:
            ...
            if "csv" in str(fpath):
                ...
                self.raw_data = Table(name, Source.DF, na_values=na_values, exclude_attr_cols=exclude_attr_cols, df=fpath)
            df = self.raw_data.df
            ...
            logging.info("Loaded %d rows with %d cells", df.shape[0], df.shape[0] * df.shape[1])

            # Call to store to database
            self.raw_data.store_to_db(self.engine.engine)
            if "csv" in str(fpath):
                status = 'DONE Loading {}'.format(str(fpath))
            else:
                status = 'DONE Loading {}'.format("DF")
    ...
    def get_repaired_dataset(self):
        ...
        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        #print(self.repaired_data.df['City'].head(20))
        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
