...
df = pd.DataFrame(default_data, columns=["Team", "City", "Country", "League"])
...
if __name__ == '__main__':
    ...
    with Pool() as pool:
        ...
        for element in results:
            counter_master += element
        print(counter_master)
        #results = [val[1] for val in list(results[0].items())]
        #print(results)
        print("Running time: " + str(time.time() - start))
        #print({k: v for k, v in sorted(x.items(), key=lambda item: item[1])})
        #print({k: v for k, v in sorted(zip(cells,results), key=lambda item: -item[1])[:5]})
