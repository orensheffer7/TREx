...
from trex_utils_hospital import naive_algo, random_combination
...
def calc_cell_shap(cell_shap, cell_repair, repeats=10, cols=None):
    ...
    for i in range(repeats):
        ...
        is_repair_with = df_repair[col_repair][i_repair] == after_fix


        df_c[col_shap][i_shap] = "NULL"

        df_repair = naive_algo(df_c, constraints)
        is_repair_without = df_repair[col_repair][i_repair] == after_fix
...
cell_repair = (18, "County")
constraints = [2, 3]

df_copy = df.copy()
row_repair, col_repair = cell_repair[0], cell_repair[1]

df_repair = naive_algo(df_copy, constraints)
...
relevant_attributes = ["Name", "City", "County"]
cells = list(itertools.product(relevant_rows.index, relevant_attributes))


results = {}

#print(list(itertools.product(df.index, df.columns)))
